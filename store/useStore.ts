...
// Migrations map: each entry migrates from version N to N+1.
const MIGRATIONS: Record<number, (oldState: any) => any> = {
  // 0 -> 1: legacy raw State -> wrapped State
  // Add future migrations here, e.g.:
  // 1: (old) => ({ ...old, myNewField: defaultValue }),
};
...
export function useStore() {
    ...
    AsyncStorage.getItem(STORAGE_KEY).then((raw) => {
      if (raw) {
        try {
          ...
          // Support two formats:
          // - legacy: raw JSON is the `State` (no schemaVersion)
          // - wrapped: { schemaVersion: number, state: State }
          let storedVersion = 0;
          let storedState: any = {};
          ...
          if (
            ...
            storedVersion = Number(parsed.schemaVersion) || 0;
            storedState = parsed.state || {};
          } else {
            // legacy format (no version) — treat as version 0
            storedVersion = 0;
            storedState = parsed;
          }

          // Apply migrations to bring storedState up to SCHEMA_VERSION
          ...
          const merged = { ...DEFAULT_STATE, ...migrated };
          ...
          // If we loaded a legacy or older version, persist the migrated, versioned shape.
          if (storedVersion !== SCHEMA_VERSION) {
            AsyncStorage.setItem(
              STORAGE_KEY,
              JSON.stringify({ schemaVersion: SCHEMA_VERSION, state: merged }),
            );
          }
        } catch {}
      }
  ...
  const persist = useCallback((next: State) => {
    return AsyncStorage.setItem(
      STORAGE_KEY,
      JSON.stringify({ schemaVersion: SCHEMA_VERSION, state: next }),
    );
  }, []);
  ...
  const togglePersonActive = useCallback(
    (id: string) => {
      setState((prev) => {
  ...
  const upsertItem = useCallback((item: Item) => {
    setState((prev) => {
      ...
      const items = isNew
        ? [...prev.items, item]
        : prev.items.map((x, i) => (i === idx ? item : x));
      const name = item.name.trim();
      const recentItems = name
        ? [
            name,
            ...prev.recentItems.filter(
              (r) => r.toLowerCase() !== name.toLowerCase(),
            ),
          ].slice(0, 50)
        : prev.recentItems;
      // Lock title on first item added
      const billTitleLocked =
        prev.billTitleLocked || (isNew && prev.items.length === 0);
      const billTitle =
        billTitleLocked && !prev.billTitleLocked ? autoTitle() : prev.billTitle;
  ...
  const startNewBill = useCallback((archiveTitle?: string) => {
    setState((prev) => {
      const activePeople = prev.activePeopleIds
        .map((id) => prev.people.find((p) => p.id === id)!)
        .filter(Boolean);
      const subtotal = prev.items.reduce((s, i) => s + (i.amount || 0), 0);
      const taxAmt =
        prev.taxType === "amount" ? prev.tax : subtotal * (prev.tax / 100);
      const serviceAmt =
        prev.serviceType === "amount"
          ? prev.service
          : subtotal * (prev.service / 100);
      const rawDiscountAmt =
        prev.discountType === "pct"
      ...
      prev.activePeopleIds.forEach((pid) => {
        perPersonShares[pid] = 0;
      });
      prev.items.forEach((it) => {
        const ppl = it.assigned.filter((pid) =>
          prev.activePeopleIds.includes(pid),
        );
        ...
        if (it.mode === "percent" && it.percents) {
          ppl.forEach((pid) => {
            perPersonShares[pid] =
              (perPersonShares[pid] || 0) +
              it.amount * ((it.percents![pid] || 0) / 100);
          });
        } else {
          const share = it.amount / ppl.length;
          ppl.forEach((pid) => {
            perPersonShares[pid] = (perPersonShares[pid] || 0) + share;
          });
        }
      });

      const newArchive: ArchivedBill[] =
        prev.items.length > 0
          ? [
              {
                id: "b" + Date.now(),
  ...
  const deleteArchivedBill = useCallback(
    (id: string) => {
      setState((prev) => {
        const next = {
          ...prev,
          archive: prev.archive.filter((b) => b.id !== id),
        };
        persist(next);
        return next;
      });
  ...
  const renameArchivedBill = useCallback(
  ...
  const resetData = useCallback(() => {
    persist(DEFAULT_STATE).then(() => setState(DEFAULT_STATE));
  }, [persist]);
      ...
      const next: State = {
        ...
        items: prev.items.map((it) => ({
          ...
          percents: it.percents
            ? Object.fromEntries(
                Object.entries(it.percents).filter(([k]) => !set.has(k)),
              )
            : undefined,
        })),
      };
...
export function computeTotals(
  ...
  const serviceAmt =
    serviceType === "amount" ? service : subtotal * (service / 100);
  const rawDiscountAmt =
  ...
  activePeopleIds.forEach((pid) => {
    perPerson[pid] = { items: 0, tax: 0, service: 0, discount: 0, share: 0 };
  });
  ...
  items.forEach((it) => {
    ...
    if (it.mode === "percent" && it.percents) {
      ...
      const share = it.amount / ppl.length;
      ppl.forEach((pid) => {
        if (perPerson[pid]) perPerson[pid].items += share;
      });
    }
  });
  ...
  activePeopleIds.forEach((pid) => {
    ...
    perPerson[pid].share =
      base +
      perPerson[pid].tax +
      perPerson[pid].service -
      perPerson[pid].discount;
  });
