...
export function ItemEditor({
  ...
  const [mode, setMode] = useState<"equal" | "percent">(
    existing?.mode || "equal",
  );
  ...
  const [percentStrs, setPercentStrs] = useState<Record<string, string>>(() => {
    ...
    if (existing?.percents)
      Object.entries(existing.percents).forEach(([k, v]) => {
        init[k] = String(v);
      });
    return init;
  });
  ...
  Object.entries(percentStrs).forEach(([k, v]) => {
    percents[k] = parseFloat(v) || 0;
  });
  ...
  const switchToPercent = () => {
    ...
    ids.forEach((id, i) => {
      initial[id] = String(even + (i === 0 ? remainder : 0));
    });
  ...
  const totalPct = Object.values(percents).reduce(
    (s, v) => s + (Number(v) || 0),
    0,
  );
        ...
        <View style={s.field}>
          ...
          <View
            style={[
              ...
              {
                backgroundColor: t.surface,
                borderColor: amountError ? "#c2410c" : t.border,
              },
            ]}
          >
            ...
            <TextInput
              ...
              value={amount}
              onChangeText={(v) => {
                // Allow only valid decimal numbers: digits and single decimal point
                ...
                const cleaned =
                  parts.length > 2
                    ...
                    : filtered;
                setAmount(cleaned);
                if (parseFloat(cleaned) > 0) setAmountError(false);
              }}
              selectTextOnFocus
            />
          </View>
      ...
      <View style={s.section}>
        <View style={s.sectionH}>
          <Text
            style={[
              s.sectionLabel,
              { color: assignedError ? "#c2410c" : t.ink3 },
            ]}
          >
          ...
          <TouchableOpacity
            onPress={() => {
              setAssigned(allOn ? [] : [...allActiveIds]);
              if (!allOn) setAssignedError(false);
            }}
          >
            <Text style={[s.linkBtn, { color: t.ink2 }]}>
          ...
          {activePeople.map((p) => {
            ...
            return (
              ...
              >
                ...
                <Text style={[s.pickName, { color: t.ink }]} numberOfLines={1}>
                  {p.name}
                </Text>
                {on && (
      ...
      <View style={s.section}>
        ...
        <View style={[s.seg, { backgroundColor: t.surface2, marginTop: 8 }]}>
          <TouchableOpacity
            style={[
              s.segBtn,
              mode === "equal" && [s.segOn, { backgroundColor: t.surface }],
            ]}
            onPress={() => setMode("equal")}
          >
            <Text
              style={[s.segText, { color: mode === "equal" ? t.ink : t.ink2 }]}
            >
              Equal
            </Text>
          </TouchableOpacity>
          <TouchableOpacity
            style={[
              s.segBtn,
              mode === "percent" && [s.segOn, { backgroundColor: t.surface }],
            ]}
            onPress={switchToPercent}
          >
            <Text
              style={[
                s.segText,
                { color: mode === "percent" ? t.ink : t.ink2 },
              ]}
            >
              Percentage
            </Text>
        ...
        {mode === "equal" && assigned.length > 0 && (
          <Text style={[s.hint, { color: t.ink2 }]}>
            Each pays {CURRENCY}
            {fmt(equalShare)}
          </Text>
        )}
        {mode === "percent" && (
          <View style={s.pctList}>
              ...
              return (
                ...
                >
                  ...
                  <TextInput
                    ...
                    onChangeText={(val) => {
                      setPercentStrs((prev) => {
                        ...
                        if (assigned.length > 1 && id !== lastId) {
                          const othersTotal = assigned
                            .filter((x) => x !== lastId)
                            .reduce(
                              (s, x) => s + (parseFloat(next[x]) || 0),
                              0,
                            );
                    ...
                    editable={
                      assigned.length < 2 ||
                      assigned[assigned.length - 1] !== id
                    }
                    selectTextOnFocus
                  />
                  <Text style={[s.affix, { color: t.ink2 }]}>%</Text>
                  <Text style={[s.pctAmt, { color: t.ink2 }]}>
                    {CURRENCY}
                    {fmt((amtNum * vNum) / 100)}
                  </Text>
                </View>
            ...
            <View
              style={[
                s.pctTotal,
                Math.round(totalPct) === 100 ? s.pctOk : s.pctBad,
              ]}
            >
              <Text
                style={{
                  fontSize: 12,
                  fontWeight: "600",
                  color: Math.round(totalPct) === 100 ? ACCENT_INK : "#c2410c",
                }}
              >
                Total · {totalPct.toFixed(0)}%
                {Math.round(totalPct) !== 100 ? " (must be 100%)" : ""}
              </Text>
            </View>
          </View>
        )}
