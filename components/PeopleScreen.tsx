...
export function PeopleScreen({
  people, activeIds, onToggle, onAdd, onUpdateTags, t,
}: {
  ...
  const toggleTag = (person: Person, tag: string) => {
    if (person.tags.includes(tag)) {
      onUpdateTags(person.id, person.tags.filter(t2 => t2 !== tag));
    } else {
  ...
  const allTagsMerged = [...new Set([...people.flatMap(p => p.tags), ...extraTags])];
    ...
    <>
      <ScrollView style={s.root} contentContainerStyle={s.content}>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} style={s.tagRow} contentContainerStyle={s.tagContent}>
          <TouchableOpacity
            style={[s.tag, { borderColor: t.border, backgroundColor: t.surface }, activeTag === 'all' && { backgroundColor: t.ink, borderColor: t.ink }]}
          ...
          {allTagsMerged.map(tag => {
            ...
            return (
              <TouchableOpacity key={tag} style={[s.tag, { borderColor: t.border, backgroundColor: t.surface }, on && { backgroundColor: t.ink, borderColor: t.ink }]} onPress={() => setActiveTag(tag)}>
          ...
          <TouchableOpacity
            ...
            onPress={() => { setShowNewTagModal(true); setNewTagInput(''); }}
          >
        ...
        <View style={s.list}>
          {filtered.map(p => {
            ...
            return (
              <View key={p.id} style={[s.card, { backgroundColor: t.surface, borderColor: on ? ACCENT : t.border }]}>
                {/* Main row */}
                <View style={s.personRow}>
                  <TouchableOpacity style={s.personTouchable} onPress={() => onToggle(p.id)}>
                    ...
                    <View style={s.personMeta}>
                      ...
                      {p.tags.length > 0 && (
                        ...
                        >
                          {p.tags.map(tag => (
                            <View key={tag} style={[s.inlinePill, { backgroundColor: t.surface2, borderColor: t.border }]}>
                              <Text style={[s.inlinePillText, { color: t.ink3 }]}>{tag}</Text>
                            </View>
                          ))}
                        </ScrollView>
                      )}
                  ...
                  <TouchableOpacity
                    style={[s.hashBtn, { borderColor: expanded ? t.ink : t.border, backgroundColor: expanded ? t.ink : 'transparent' }]}
      ...
      >
        ...
        >
          <TouchableOpacity style={s.modalBackdrop} activeOpacity={1} onPress={() => setShowNewTagModal(false)} />
          <View style={[s.modalBox, { backgroundColor: t.surface, borderColor: t.border }]}>
            <Text style={[s.modalTitle, { color: t.ink }]}>New tag</Text>
            <TextInput
              style={[s.modalInput, { backgroundColor: t.bg, borderColor: t.border, color: t.ink }]}
            ...
            <View style={s.modalActions}>
              <TouchableOpacity style={[s.modalCancel, { borderColor: t.border }]} onPress={() => setShowNewTagModal(false)}>
                <Text style={[s.modalCancelText, { color: t.ink2 }]}>Cancel</Text>
              </TouchableOpacity>
              <TouchableOpacity
                style={[s.modalConfirm, { backgroundColor: t.ink, opacity: newTagInput.trim() ? 1 : 0.35 }]}
                onPress={confirmNewTag}
                disabled={!newTagInput.trim()}
              >
                <Text style={[s.modalConfirmText, { color: t.bg }]}>Create tag</Text>
              </TouchableOpacity>
            </View>
          </View>
        </KeyboardAvoidingView>
...
const s = StyleSheet.create({
  ...
  personRow: { flexDirection: 'row', alignItems: 'center', gap: 10, paddingRight: 14, paddingVertical: 10 },
  personTouchable: { flex: 1, flexDirection: 'row', alignItems: 'center', gap: 12, paddingLeft: 14 },
  ...
  inlinePill: { height: 18, paddingHorizontal: 7, borderRadius: 9, borderWidth: 1, alignItems: 'center', justifyContent: 'center' },
  inlinePillText: { fontSize: 10, textTransform: 'capitalize' },

  hashBtn: { width: 26, height: 26, borderRadius: 8, borderWidth: 1, alignItems: 'center', justifyContent: 'center' },
  ...
  tagPanel: { paddingHorizontal: 14, paddingBottom: 12, paddingTop: 10, borderTopWidth: 1 },
  tagPills: { flexDirection: 'row', flexWrap: 'wrap', gap: 6 },
  tagPill: { height: 28, paddingHorizontal: 12, borderRadius: 14, borderWidth: 1, alignItems: 'center', justifyContent: 'center' },
  ...
  modalBox: { borderTopLeftRadius: 20, borderTopRightRadius: 20, borderWidth: 1, padding: 24, gap: 14 },
  modalTitle: { fontSize: 16, fontWeight: '700' },
  modalInput: { height: 44, borderRadius: 12, borderWidth: 1, paddingHorizontal: 14, fontSize: 14 },
  modalActions: { flexDirection: 'row', gap: 10 },
  modalCancel: { flex: 1, height: 44, borderRadius: 12, borderWidth: 1, alignItems: 'center', justifyContent: 'center' },
  modalCancelText: { fontSize: 14, fontWeight: '500' },
  modalConfirm: { flex: 1, height: 44, borderRadius: 12, alignItems: 'center', justifyContent: 'center' },
