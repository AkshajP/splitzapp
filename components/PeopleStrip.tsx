...
export function PeopleStrip({
  people, activeIds, onPick, t,
}: {
  people: Person[]; activeIds: string[]; onPick: () => void; t: T;
}) {
  const active = activeIds.map(id => people.find(p => p.id === id)!).filter(Boolean);
  return (
    <View style={[s.strip, { borderBottomColor: t.border }]}>
      <TouchableOpacity style={[s.addBtn, { borderColor: t.borderStrong }]} onPress={onPick}>
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} style={s.scroll} contentContainerStyle={s.scrollContent}>
        {active.length === 0 && (
          <Text style={[s.empty, { color: t.ink3 }]}>Tap + to add people</Text>
        )}
        {active.map(p => (
          <View key={p.id} style={[s.chip, { backgroundColor: t.surface, borderColor: t.border }]}>
...
const s = StyleSheet.create({
  strip: { flexDirection: 'row', alignItems: 'center', gap: 10, paddingHorizontal: 16, paddingVertical: 14, borderBottomWidth: 1 },
  addBtn: {
  ...
  chip: {
    ...
    paddingLeft: 6, paddingRight: 12, borderRadius: 30, borderWidth: 1, height: 44,
  },
